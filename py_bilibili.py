...
class Spider(Spider):
    ...
    def __init__(self):
        ...
        if self.config["filter"].get('收藏'):
            for i in self.config["filter"].get('收藏'):
                if i['key'] == 'mlid':
                    i['value'] = fav_list
        url = 'https://api.bilibili.com/x/relation/followings?vmid=%s&order=desc&order_type=attention&ps=10' % self.userid
        rsp = self.fetch(url, headers=self.header, cookies=self.cookies)
        content = rsp.text
        jo = json.loads(content)
        follow_list = []
        if jo['code'] == 0:
            for follow in jo['data'].get('list'):
                follow_dict = {
                    'n': follow['uname'].replace("<em class=\"keyword\">", "").replace("</em>", "").replace("&quot;",
                                                                                                         '"').strip(),
                    'v': follow['mid']}
                follow_list.append(follow_dict)
        if self.config["filter"].get('关注'):
            for i in self.config["filter"].get('关注'):
                if i['key'] == 'tid':
                    i['value'] = follow_list
    ...
    def post_history(self, aid, cid):
        ...
        requests.post(url=url, headers=self.header, cookies=self.cookies)
    ...
    def homeVideoContent(self):
        result = {}
        videos = self.get_rank(pg=1)['list'][0:5]
        result['list'] = videos
        return result
        ...
        if 'ugc_season' in jRoot['data']:
            ...
            for section in sections:
                ...
                for episode in episodes:
                    ...
                    playUrl = playUrl + '{0}${1}_{2}#'.format(ep_title, did, cid)
                if sec_len > 1:
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        url1 = 'https://api.bilibili.com:443/x/player/playurl?avid={0}&cid={1}&qn=116'.format(ids[0], ids[1])
        url2 = 'https://api.bilibili.com/pgc/player/web/playurl?qn=116&aid={0}&cid={1}'.format(ids[0], ids[1])
        urls = (url1, url2)
        for url in urls:
            rsp = self.fetch(url, cookies=self.cookies, headers=self.header)
            jRoot = json.loads(rsp.text)
