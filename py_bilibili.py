...
class Spider(Spider):
    ...
    def get_Login_qrcode(self):
        ...
        if jo['code'] == 0:
            ...
            url = 'https://tool.lu/qrcode/basic.html?text=https%3A%2F%2Fpassport.bilibili.com%2Fh5-app%2Fpassport%2Flogin%2Fscan%3Fnavhide%3D1%26qrcode_key%3D{0}%26from%3D&tolerance=30&size=200&margin=50&front_color=%23000000&background_color=%23ffffff'.format(id)
    ...
    def get_history(self, type, pg):
        ...
        if jo['code'] == 0:
            ...
            for vod in vodList:
                if 'history' in vod:
                    business = vod['history']['business']
                    aid = str(vod['history']['oid']).strip()
                    img = vod['cover'].strip()
                    part = str(vod['history']['part']).strip()
                else:
                    business = 'archive'
                    aid = str(vod["aid"]).strip()
                    img = vod['pic'].strip()
                ...
                title = vod['title'].replace("<em class=\"keyword\">", "").replace("</em>", "").replace("&quot;", '"')
                if business == 'live':
                    remark = str(vod['live_status']).replace("0", "未开播  ").replace("1", "直播中  ") + vod['author_name'].strip()
                else:
                    # 获取视频总时长
    ...
    def get_vod_relation(self, aid, relation):
        url = 'https://api.bilibili.com/x/web-interface/archive/relation?aid={0}'.format(aid)
        rsp = self.fetch(url, headers=self.header, cookies=self.cookies)
        jo = json.loads(rsp.text)
        if jo['code'] == 0:
            jo = jo['data']
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if tid == "推荐":
            ...
            if 'mid' in extend:
                mid = extend['mid']
            if not mid and not self.isLogin or mid == '登录':
                return self.get_Login_qrcode()
    ...
    def get_normal_episodes(self, episode):
        ...
        if ep_from and ep_from == 'bangumi':
            ...
            if badge and self.cookies_vip and self.hide_bangumi_vip_badge:
                badge = badge.replace('会员', '')
            if badge:
                ep_title = '【' + badge + '】' + ep_title
            long_title = episode.get('long_title')
            if not long_title.isdigit():
                if not '【' in ep_title:
                    ep_title += ' '
                ep_title += long_title
        ...
        playUrl = '{0}${1}_{2}{3}{4}{5}'.format(ep_title, aid, cid, ssid, epid, duration)
        return playUrl
    ...
    def get_vod_related(self, jo_Related):
        ...
        self.detailContent_args['relatedP'] = ['#'.join(playUrl)]
    ...
    def get_vod_pages(self, pages):
        ...
        self.detailContent_args['firstP'] = ['#'.join(playUrl)]
    ...
    def detailContent(self, array):
        ...
        relation = []
        t = threading.Thread(target=self.get_vod_relation, args=(aid, relation, ))
        t.start()
        url = "https://api.bilibili.com/x/web-interface/view/detail?aid={0}".format(aid)
        rsp = self.fetch(url, headers=self.header, cookies=self.cookies_fake)
        jRoot = json.loads(rsp.text)
        jo = jRoot['data']['View']
        ...
        vod['vod_director'] = up_name + '　' + relation
    ...
    def ysContent(self, array):
        ...
        for tmpJo in jo['episodes']:
            ...
            if badge:
                part += '【' + badge + '】'
            long_title = tmpJo['long_title']
            if not long_title.isdigit():
                if not '【' in part:
                    part += ' '
                part += long_title
            part = part.replace("#", "﹟").replace("$", "﹩")
            url = '{0}${1}_{2}_ss{3}_ep{4}_dur{5}#'.format(part, aid, cid, self.detailContent_args['ssid'], epid, duration)
            if preview:
                playUrl_preview += url
                continue
            playUrl += url
        ...
        PreviewPu = []
        if playUrl_preview:
            FirstPf = PreviewPf = ['预告']
            FirstPu = PreviewPu = [playUrl_preview]
        EndPf = []
        EndPu = []
        #非正片花絮
        if section:
            self.get_section_event.wait()
            EndPf = self.detailContent_args['SectionPf']
            EndPu = self.detailContent_args['SectionPu']
        #系列
        if seasons:
            EndPf = ['更多系列'] + EndPf
            EndPu = ['更多系列在快速搜索中查看$ '] + EndPu
        #正片
        ThisPf = []
        ThisPu = []
        if playUrl:
            ThisPf = [self.detailContent_args['s_title']]
            ThisPu = [playUrl]
            FirstPf = ThisPf + PreviewPf
            FirstPu = ThisPu + PreviewPu
        fromL = FirstPf + EndPf
        urlL = FirstPu + EndPu
            ...
            for f in format:
                for c in codec:
                    for n in range(2):
                        ...
                        if n > 0:
                            name = '备线' + str(n)
                        url.append(f + '_' + c + name + '$2_' + qn[q] + '_' + format[f] + '_' + codec[c] + '_' + str(n) + '_' + str(room_id))
            url = '#'.join(url)
            api2_playUrl.append(url)
    ...
    def searchContent(self, key, quick):
        self.search_key = key
        if quick and self.up_mid and self.detailContent_args['from'] != 'bangumi':
            if self.up_mid != self.get_up_videos_mid:
        ...
        if not self.get_search_content_event.is_set():
            return
        if quick:
            if self.detailContent_args['from'] == 'bangumi':
    ...
    def start_set_cookie(self, key, vip):
        url = 'https://passport.bilibili.com/x/passport-login/web/qrcode/poll?qrcode_key={0}'.format(key)
        rsp = self.fetch(url=url, headers=self.header, cookies=self.cookies_fake)
    ...
    def unset_cookie(self, num):
        ...
        if int(num):
            ...
            self.cookies = {'buvid3': str(uuid4()) + 'infoc'}
            self.isLogin = 0
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if 'setting' in id:
            if 'login' in id:
                ...
                num = aid
                t = threading.Thread(target=self.unset_cookie, args=(num, ))
                t.start()
            return result
        if 'zhui' in id:
