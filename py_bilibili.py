            ...
            if jo['code'] == 0:
                ...
                for vod in vodList:
                    if vod['type'] == 'DYNAMIC_TYPE_AV':
                        ...
                        avod = vod['modules']['module_author']
                        aid = str(ivod['aid']).strip()
                        title = ivod['title'].strip().replace("<em class=\"keyword\">","").replace("</em>","")
                        img =  ivod['cover'].strip()
                        remark = str(ivod['duration_text']).strip() + '  ' + str(avod['name']).strip()
                        videos.append({
                            "vod_id":aid,
                            "vod_name":title,
                            "vod_pic":img,
...
#每周必看
    def get_weekly(self):
        result = {}
        url= 'https://api.bilibili.com/x/web-interface/popular/series/one?number=189' #number后面数字可改
        rsp = self.fetch(url,cookies=self.getCookie())
        content = rsp.text
        jo = json.loads(content)
        if jo['code'] == 0:
...
#分区列表
    def categoryContent(self,tid,pg,filter,extend):	
        ...
        if tid == "入站必刷":
            return self.get_must()
        if tid in ("1" , "3", "129", "4","119","36","188","234", "160","211","217","223","155","5","181"):
...
#搜索
    def searchContent(self,key,quick):
        search = self.categoryContent(tid=key,pg=2,filter=None,extend=None)
        result = {
            'list':search['list']
        }
        return result
